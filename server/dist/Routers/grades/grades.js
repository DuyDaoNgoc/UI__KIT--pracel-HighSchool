...
// ==============================
router.get("/status", authMiddleware_1.verifyToken, async (_req, res) => {
    try {
        let setting = await Setting_1.default.findOne({ key: SETTINGS_KEY });
        if (!setting) {
...
// ==============================
router.post("/lock", authMiddleware_1.verifyToken, (0, authMiddleware_1.checkRole)(["admin"]), async (_req, res) => {
    try {
        const setting = await Setting_1.default.findOneAndUpdate({ key: SETTINGS_KEY }, { value: { locked: true } }, { upsert: true, new: true });
        res.json({ locked: !!setting?.value?.locked });
    }
    catch (err) {
...
// ==============================
router.post("/unlock", authMiddleware_1.verifyToken, (0, authMiddleware_1.checkRole)(["admin"]), async (_req, res) => {
    try {
        const setting = await Setting_1.default.findOneAndUpdate({ key: SETTINGS_KEY }, { value: { locked: false } }, { upsert: true, new: true });
        res.json({ locked: !!setting?.value?.locked });
    }
    catch (err) {
...
// ==============================
router.post("/toggle-lock", authMiddleware_1.verifyToken, (0, authMiddleware_1.checkRole)(["admin"]), async (req, res) => {
    try {
        const { locked } = req.body;
        if (locked === undefined)
            return res.status(400).json({ message: "Missing locked value" });
        const setting = await Setting_1.default.findOneAndUpdate({ key: SETTINGS_KEY }, { value: { locked: !!locked } }, { upsert: true, new: true });
        res.json({ locked: !!setting?.value?.locked });
    }
    catch (err) {
...
// ==============================
router.post("/request-update", authMiddleware_1.verifyToken, (0, authMiddleware_1.checkRole)(["teacher"]), async (req, res) => {
    const authReq = req;
    ...
    try {
        ...
        const student = await User_1.User.findOne({ studentId });
...
// ==============================
router.get("/class/:classId", authMiddleware_1.verifyToken, (0, authMiddleware_1.checkRole)(["admin", "teacher"]), async (req, res) => {
    try {
        const { classId } = req.params;
        // Tìm tất cả học sinh trong lớp
        const students = await User_1.User.find({ class: classId, role: "student" });
        // Tìm giáo viên phụ trách lớp
        const teacherIds = students.map((s) => s.teacherId).filter(Boolean);
        const teacher = teacherIds.length
            ? await User_1.User.findOne({ role: "teacher", _id: { $in: teacherIds } })
            : null;
        res.json({
            classId,
