...
const createTeacher = async (req, res) => {
    try {
        ...
        const { teacherId, name, dob, phone, major, subject, address, residence, email, classAssigned = [], // lớp được phân công
         } = req.body;
        // ✅ Kiểm tra dữ liệu bắt buộc
        if (!teacherId || !name || !dob || !phone || !major || !subject) {
            return res.status(400).json({
                success: false,
                message: "Thiếu thông tin bắt buộc (teacherId, name, dob, phone, major, subject).",
            });
        }
        // ✅ Validate ngày sinh
